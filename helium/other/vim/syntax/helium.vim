...
syn keyword heKeyword and begin data effect elif else end export extern
syn keyword heKeyword finally fn forall handle handler if import in
syn keyword heKeyword include let match module of
syn keyword heKeyword open rec return sig signature struct then this type val with
syn keyword heUKeyword Effect Sig Type
...
syn match   heType "\<[A-Z][0-9a-zA-Z_']*\>" contains=heUKeyword
syn match   heSpecial "[]{}[|:,.;]\|->\|=>\?\|\(//\@!\)"
syn match   heCharLit "'\([^\\\n]\|\\\(['\"\\ntbr]\)\)'"
...
hi def link heUKeyword     Special
