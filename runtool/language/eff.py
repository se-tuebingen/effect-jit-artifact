...
class EffBackend(Language):
    ...
    def compile(self, path: str, name: str) -> list[str] | None:
        ...
        result_path = os.path.abspath(f"./.eff-out/{uniq_prefix}")
        ...
        eff = os.path.abspath("./eff/eff.exe")
        ...
        e_output = tee_stderr(e_proc, "[blue]eff  [/blue]|")

        if self.backend == "plain-ocaml":
            # Put fixture in place
            shutil.copy("./eff/ocamlHeader/ocamlHeader.ml", os.path.join(result_path, "./OcamlHeader.ml"))
            shutil.copy(os.path.join(os.path.dirname(path), "./wrapper.ml"), os.path.join(result_path, "./wrapper.ml"))

            # Ocamlformat
            f_proc = run_to_file(["opam", "exec", "--", "ocamlformat", result_path + f"/{fname}.{ext}", "--enable-outside-detected-project"], result_path + f"/generated.ml", cwd=result_path)
            f_output = tee_stderr(f_proc, "[blue]  fmt [/blue]|")

            # Ocamlopt
            o_proc = run(["opam", "exec", "--", "ocamlopt", "-O3", "-o", result_path + f"/{fname}", "OcamlHeader.ml", "generated.ml", "wrapper.ml"], cwd=result_path)
