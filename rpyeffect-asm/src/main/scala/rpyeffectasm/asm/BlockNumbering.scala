...
class BlockNumbering[F <: AsmFlags, Tag <: Id, Var <: Id, OTpe]
  extends Phase[Program[F,Tag,Id,Var,OTpe], Program[F,Tag,Index,Var,OTpe]] {
  ...
  def apply(c: Clause[F, Id, Var, OTpe])(using Numbering): Clause[F, Index, Var, OTpe] = c match {
    case Clause(params, capts, target) => Clause(params, capts, apply(target))
  }
  def apply(i: Instruction[F, Tag, Id, Var, OTpe])(using Numbering): Instruction[F, Tag, Index, Var, OTpe] = i match {
    ...
    case i: Instruction[F, Tag, Nothing, Var, OTpe] @unchecked => i
  }
  def apply(b: Block[F, Tag, Id, Var, OTpe])(using Numbering): Block[F, Tag, Index, Var, OTpe] = b match {
    case Block(label, params, ret, instructions, export_as) => Block(apply(label), params, ret, instructions map apply, export_as)
  }
  override def apply(p: Program[F, Tag, Id, Var, OTpe])(using ErrorReporter): Program[F, Tag, Index, Var, OTpe] = p match {
    case Program(blocks) =>
